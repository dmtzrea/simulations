This is the code corresponding to simulation 1 in the thesis. 
...
The results are already in the folder, so one can run lines 1 - 54, and then 136 until the 
end of the script. This way one skips the simulation itself, which can take between 
3 and 8 days to complete, depending on the machine it is run on, and use the simulation
results already generated instead. If one wishes to reproduce the simulation results, one can run the entire script.
The results obtained should be identical with the results reported on the thesis, due to the 
random seed setup. 

The script produces the sigma plots, the bias plots, and the MSE plots reported on the thesis.
