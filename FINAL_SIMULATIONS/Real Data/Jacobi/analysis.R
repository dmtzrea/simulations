...
ggsave(burn %>% ggplot(aes(x = Z4/100, y = T2) )+ geom_point(aes(color = as.factor(D2)), size = 0.7) + 
...
ggsave(burn %>% ggplot(aes(x = Z4/100, y = T1) )+ geom_point(aes(color = as.factor(D1)), size = 0.7) + 
         facet_wrap(Z2 ~., ncol = 2, labeller = labeller(Z2 = labs)) + 
         theme_bw() + theme(panel.border = element_blank(), panel.grid.major = element_blank(),
                            panel.grid.minor = element_blank(), axis.line = element_line(colour = "black"),
                            legend.position = 'bottom') +
