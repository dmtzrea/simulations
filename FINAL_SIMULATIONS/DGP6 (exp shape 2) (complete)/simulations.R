...
# LOAD RESULTS FROM DIFFERENT RUNS (ONLY IF NOT EVERYTHING RAN ON THIS MACHINE) ----
load("results_1__4.Rdata",  temp_env <- new.env())
results1 <- as.list(temp_env)[[1]]
load("results2.Rdata",  temp_env <- new.env())
results2 <- as.list(temp_env)[[1]][c(5,6,7)]
load("results3.Rdata",  temp_env <- new.env())
results3 <- as.list(temp_env)[[1]][c(8,9,10)]
h_list = c(results1, results2, results3)
