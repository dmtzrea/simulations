...
matrix = matrix(c(-5/3, 15, 0.3, 200, 500,-3.5, 6.5, 0.7, 200, 500,-5/3, 5.5, 0.3, 200, 500,-3, 3, 0.5, 200, 500),
...
datasets = list(array(dim = c(matrix[1, 4],6,500)) ,array(dim = c(matrix[2, 4], 6, 500)), array(dim = c(matrix[3, 4], 6, 500)),
                array(dim = c(matrix[4, 4], 6, 500)))
...
for(k in 1:nrow(matrix)){
  for (i in 1:dim(datasets[[k]])[3]){
    ...
    rm(list=c("tau","min","max","n","N","epsilon_raw","x","X","X_s","epsilon","T",
              "C","Y","Delta"))
  }
}
